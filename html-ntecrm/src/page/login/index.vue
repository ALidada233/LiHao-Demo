<template>
  <div
    ...
    style="background:url('https://img.alicdn.com/tfs/TB1zsNhXTtYBeNjy1XdXXXXyVXa-2252-1500.png') center center no-repeat; background-size:100% 100%;"
  >
    <div style="position:absolute;left: 25%;z-index: 1000;margin-top: 250px;">
      <h2 style="color:#fff;font-size: 38px;">欢迎使用</h2>
      <h3 style="color:#fff;font-size: 35px;margin-top: 10px;">CRM客户管理系统</h3>
    </div>
    <el-form
      ref="loginForm"
      :model="loginForm"
    ...
    >
      ...
      <div style="text-align:center;">
        <p style="color:red;">首次登录请输入手机验证码</p>
      </div>
      <el-button
        type="primary"
...
export default {
  ...
  methods: {
    ...
    handleLogin() {
      // window.localStorage.setItem("username", this.loginForm.username); //存储用户名
      // window.localStorage.setItem("password", this.loginForm.password); //存储密码
      var _this = this;
            ...
            res => {
              if (res.msgCode == 200) {
                ...
                if (permissionList.length > 0) {
                  routesArr = this.recursionRouter(permissionList);
                }
                sessionStorage.setItem("routesArr", JSON.stringify(routesArr));
                //默认跳转的第一个路由
                let firstRouter = "/admin/" + routesArr[0].path;
    ...
    recursionRouter(userRouter = []) {
      ...
      var realRoutes = [];
      dynamicRouter.forEach((v, i) => {
        userRouter.forEach((item, index) => {
          if (item.url === v.name) {
            if (item.children && item.children.length > 0) {
              realRoutes = realRoutes.concat(
                this.recursionRouter(item.children, dynamicRouter)
              );
            }
            v.pName = item.name;
            realRoutes.push(v);
  ...
  created() {
    ...
    // this.loginForm.username = window.localStorage.getItem("username"); //读取用户名
    // this.loginForm.password = window.localStorage.getItem("password"); //读取用户密码
    // localStorage.clear(); //获取完数据后将localstorage清空
  }
};
</script>
