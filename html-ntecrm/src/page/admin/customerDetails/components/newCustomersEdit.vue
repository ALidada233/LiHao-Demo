  ...
  <div class="edit" @click.self="closeDialog">
    <div class="edit-content">
      ...
      <el-form :model="form" ref="form" style="height:520px;overflow:scroll;" :rules="rules">
        ...
        <el-form-item class="formSty" prop="cusRegionIds" label="客户区域" :label-width="formLabelWidth">
        ...
        <el-form-item v-if="editState" label="拜访记录" class="formSty" style="width:100%;" :label-width="formLabelWidth">
          <el-tooltip class="item" effect="dark" content="新增" placement="top" style="margin-bottom:10px">
            <el-button type="primary" size="medium" style="margin-top: 10px;margin-left: 0;" icon="el-icon-plus" @click="visitVisible=true;visitStatus=false;"></el-button>
          </el-tooltip>
          <el-table :data="getCustomerVisitList" :header-cell-style="{'background-color': '#EFEDF0','height': '30px','padding':'0'}"
            style="width: 100%">
            <el-table-column prop="conName" label="操作" align="center" width="150">
              <template slot-scope="scope">
                <el-button size="mini" round type="primary" icon="el-icon-edit" @click="visitVisible=true;visitStatus=true;visitform=JSON.parse(JSON.stringify(scope.row));"></el-button>
              </template>
            </el-table-column>
            <el-table-column prop="custCode" label="客户编码" width="120">
            </el-table-column>
            <el-table-column prop="remark" label="拜访内容" >
            </el-table-column>
    ...
    <!-- 添加或编辑拜访记录 -->
    <el-dialog
      :title="visitStatus?'编辑拜访记录':'新增拜访记录'"
      :visible.sync="visitVisible"
      v-if='visitVisible'
      width="50%"
      :append-to-body="true"
      :before-close="visitHandleClose">
      <el-form ref="visitform" :model="visitform" label-width="80px">
        <el-form-item label="拜访记录">
          <el-input type="textarea" v-model="visitform.remark" style="width:85%;" :rows="12"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="visitHandleClose">取 消</el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        getCustomerVisitList:[],//拜访客户记录数量
        visitform:{},//拜访记录内容
        visitStatus:false,//拜访记录的编辑状态true为编辑false新增
        visitVisible:false,//拜访弹框
        rules: {
          custName: {
            required: true,
            trigger: 'blur',
    ...
    methods: {
      ...
      getCustomerVisit(){ //获取拜访记录
        getCustomerVisit({custCode:this.form.custCode},res=>{
          this.getCustomerVisitList=res.data.list;
        })
      },
      visitHandleClose(){ //关闭新增或编辑拜访记录窗口
        this.visitform={};
      ...
      submitVisit(){  //保存跟进记录
        let params={
          ...
          remark :this.visitform.remark
        }
        if(this.visitStatus){
          params.id=this.visitform.id
        }
        saveCustomerVisit(params,res=>{
          if (res.msgCode == 200) {
            this.$message({
              message: this.visitStatus?'编辑拜访记录成功':'新增拜访记录成功',
              type: 'success'
            });
            this.getCustomerVisit();
            this.visitHandleClose();
          } else {
           this.$message({
              message: this.visitStatus?'编辑拜访记录失败':'新增拜访记录失败',
              type: 'warning'
            });
          }
        })
