<template>
  <div class="newCustomers" v-if="dialogFormVisible">
    <el-dialog
      ...
      >
        <div class="left" style="width:45%">
          ...
          <el-form-item label="联系人电话" :label-width="formLabelWidth" prop="telePhone">
            ...
            ></el-input>
            <br />
            <span style="color:red">{{echoMsg}}</span>
          </el-form-item>
...
import { checkCustPhone } from "util/req/customer/newUser";
export default {
  name: "newCustomers",
  props: {
    dialogFormVisible: {
  ...
  watch: {
    "form.telePhone": function(val) {
      ...
      console.log("isPhoneTrue1" + this.isPhoneTrue);
      if (val == null || val == "" || val == this.resultPhone) {
        return;
      }
      this.isPhoneTrue = false;
      console.log("isPhoneTrue2" + this.isPhoneTrue);
    },
    dialogFormVisible() {
    ...
    currrentLink(val) {
      //判断是否为编辑状态，否的情况下清除表单的内容，防止上次编辑点击新增会有输入内容
      if (val == null) {
        this.resetLInkForm();
      }
    }
  ...
  methods: {
    ...
    confirmHandle() {
      console.log("isPhoneTrue3" + this.isPhoneTrue);
      if (this.isPhoneTrue) {
    ...
    checkCustPhone() {
      ...
      if (this.form.telePhone == this.resultPhone) {
        ...
        console.log("isPhoneTrue4" + this.isPhoneTrue);
        return;
      }
      checkCustPhone(
        {
        ...
        res => {
          ...
          console.log(this.isPhoneTrue + "==isPhoneTrue5");
        }
      );
    }
  }
