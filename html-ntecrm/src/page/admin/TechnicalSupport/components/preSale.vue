  ...
  <div>
    <div class="content">
      ...
      <div style="display:inline-flex;align-items: flex-start">
        ...
        <el-select
          ...
          :filterable="true"
          :clearable="true"
          style="width:150px;display:inline-flex;margin-left:10px;"
        >
          <el-option
      ...
      <el-table
        ...
        height="490"
        class="preSale"
        :header-cell-style="{'background':'#E6EBF0'}"
        style="width: 100%;"
      >
        <el-table-column label="操作" width="130" align="center">
          <template slot-scope="props">
            ...
            <el-button
              ...
              type="success"
              icon="el-icon-check"
              @click="completedSupport(props.row.id)"
            ></el-button>
          </template>
        </el-table-column>
        <el-table-column label="发起人" align="center" width="78" prop="createName"></el-table-column>
        <el-table-column label="协助内容" align="center" prop="remark" width="300">
        ...
        <el-table-column label="需求附件" align="center" prop="demandCode" width="80">
          <template slot-scope="scope">
            <!-- <span
              v-if="scope.row.optFile"
              @click="previewFileVisible=true;previewFileUrl=scope.row.optFile"
              style="color:#409EFF;cursor:pointer;"
            >预览</span>-->
            <el-tag v-if="scope.row.optFile" type="success">有</el-tag>
            <span v-else>无</span>
          </template>
        </el-table-column>
        <el-table-column width="100" align="center" label="处理人">
          <template slot-scope="scope">
            <span
              v-for="(item,index) in (scope.row.readName==null?null:scope.row.readName).split(',')"
              :key="index"
            >
              <span v-if="item.indexOf('未读')>=0" style="color:red">{{item}}</span>
        ...
        <div v-if="this.type==1">
          <el-table-column label="需求内容" align="center" prop="demandInfo" width="300">
            <template slot-scope="scope">
              <div>
                <el-input
                  type="textarea"
                  ...
                  disabled
                  style="color:#333;"
                  v-model="scope.row.demandInfo"
                ></el-input>
              </div>
            </template>
          </el-table-column>
        </div>
        <el-table-column label="客户信息" align="center" prop="custInfo" width="180"></el-table-column>
      ...
      <transition name="fade-transform" mode="out-in">
        <discuss
          ...
          @closeVisilog="discussStatus=false;queryTechnicalSupportList();$parent.getTechnicalSupportIndex()"
          @refresh="$parent.getTechnicalSupportIndex()"
        ></discuss>
      </transition>
    </div>
    <el-dialog title="附件" :visible.sync="previewFileVisible" width="60%" :append-to-body="true">
      <div style="height:400px;overflow-y:scroll;text-align:center;">
        <object
          v-if="previewFileUrl&&previewFileUrl.toLowerCase().indexOf('.pdf')>-1"
          :data="previewFileUrl"
          width="100%"
          height="400px"
