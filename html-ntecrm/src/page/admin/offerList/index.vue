  ...
  <div class="customerDemand" id="customerDemand" ref="customerDemand">
    <div style="padding:10px;">
      <div class="left" style="margin-bottom: 0px;">
        ...
        <el-select
          no-data-text="加载中"
          clearable
          v-model="selectLinkMan"
          filterable
          @change="searchLinkName(selectLinkMan)"
          placeholder="请选择公司"
        >
          <el-option
      ...
      <div class="right" style="margin-bottom: 5px;">
        <el-button
          v-tip.top.dark="'返回客户管理'"
          size="medium"
          @click="$router.push({ path: '/admin/customer', query: {condition:searchCriteria}})"
          icon="el-icon-back"
        ></el-button>
        <el-button
          v-tip.top.dark="'新增需求'"
          type="primary"
          size="medium"
          @click="needVisible=true;"
          icon="el-icon-plus"
        ></el-button>
        <el-button
          v-tip.top.dark="'生成报价单'"
          style="background:#5bc0de;color:#fff;"
          size="medium"
          @click="contractDialogVisible=true;contractType='offer'"
          icon="el-icon-document"
        ></el-button>
        <el-button
          v-tip.top.dark="'生成合同'"
          style="background:#8E8CBC;color:#fff;"
          size="medium"
          @click="contractDialogVisible=true;contractType='contract'"
      ...
      >
        <el-table-column label="操作" width="195">
          <template slot-scope="props" v-if="isShow">
            <el-button
              ...
              type="primary"
              size="mini"
              @click="offerHandle(props.row.id,props.row.status)"
            ...
            <el-button
              ...
              style="margin-bottom:5px;background:#5cb85c;color:#fff;"
              class="iconfont"
              round
            >&#xe715;</el-button>
            <el-button
              v-tip.top.dark="'预览报价单'"
              size="mini"
              @click="offerHandle(props.row.id,props.row.status,props.row,'offer')"
            ...
            <el-button
              v-tip.top.dark="'预览合同'"
              size="mini"
              @click="offerHandle(props.row.id,props.row.status,props.row)"
              v-if="(props.row.status==5) && props.row.contractFile!==null && props.row.contractFile!==''"
              style="margin-bottom:5px;background:#8085E9;color:#fff;"
              class="iconfont"
              round
            >&#xe617;</el-button>
            <el-button
              ...
              v-tip.top.dark="'作废'"
              size="mini"
              @click="cancelId=props.row.id;cancelVisible=true;"
          ...
          <template slot-scope="scope">
            <el-badge :is-dot="scope.row.technicalNum>0" class="item">
              <div v-if="scope.row.type==1" :style="{'color':scope.row.indexId>0?'#409EFF':''}">
                <div
                  v-if="infoActive==scope.$index&&scope.row.demandInfos.length>1"
                  style="cursor: pointer;"
                >
                  <div
                    v-for="(item,index) in scope.row.demandInfos"
                    :key="index"
                    @click="getFollowList(scope.row.id,scope.row)"
                ...
                <span v-else style="cursor: pointer;text-align:left;">
                  <p
                    v-for="(item,index) in scope.row.demandInfos[0]"
                    :key="index"
                    @click="getFollowList(scope.row.id,scope.row)"
                  >{{item}}</p>
                  <i
                    class="el-icon-arrow-down"
                    v-if="scope.row.demandInfos.length>1"
                    @click="infoActive=scope.$index"
                  ></i>
                </span>
              </div>
              <div
              ...
              >
                <span
                  :style="{'color':scope.row.indexId>0?'#409EFF':''}"
                  v-if="scope.row.type==2"
                >{{scope.row.demandContent}}</span>
              </div>
            </el-badge>
          </template>
        </el-table-column>
            ...
            <span>
              ...
              >
                <span
                  ...
                  v-html="$root.OmissionText(scope.row.companyName==null?null:scope.row.companyName,8)"
                ></span>
              </el-popover>
            </span>
          </template>
        ...
        <el-table-column label="联系人信息" align="center" width="190">
          <template slot-scope="scope">
            <i
              ...
              @click="editContactsVisible=true;demandId=scope.row.id"
            ></i>
            <span
              style="color:#409EFF;cursor:pointer;"
              @click="$router.push({ path: '/admin/customer', query: {name:scope.row.companyName}})"
            >
              {{scope.row.contacts}}-{{scope.row.phone}}
              <br />
              {{scope.row.email}}
            </span>
          </template>
        ...
        <el-table-column label="状态" align="center" width="80" prop="status">
          <template slot-scope="scope">
            ...
            <el-tag color="#B9DBF9" v-if="scope.row.status==2">已报价</el-tag>
        ...
        <el-table-column label="备注" width="200" align="center" prop="remark"></el-table-column>
        <el-table-column label="业务员信息" align="center" width="250">
          <template slot-scope="scope">
            <span>
              {{scope.row.saleName}}-{{scope.row.salePhone}}
              <br />
              {{scope.row.saleEmail}}
            </span>
          </template>
        </el-table-column>
        <el-table-column label="操作人" align="center" width="78" prop="createName"></el-table-column>
        <el-table-column label="需求编号" align="center" prop="demandCode" width="127"></el-table-column>
        <el-table-column label="需求附件" align="center" prop="demandCode" width="80">
          <template slot-scope="scope">
            <span
              v-if="scope.row.demandFile"
              @click="previewFileVisible=true;previewFileUrl=scope.row.demandFile"
              style="color:#409EFF;cursor:pointer;"
            >预览</span>
            <span v-else>无</span>
          </template>
        </el-table-column>
        <el-table-column label="更新时间" width="180" align="center" prop="updateTime"></el-table-column>
        <el-table-column label="创建时间" width="180" align="center" prop="createTime"></el-table-column>
    ...
    <transition name="fade-transform" mode="out-in">
      <add-follow
        class="addFollow"
        ref="addFollow"
        v-if="toggleFollowVisible"
        :toggleFollowVisible="toggleFollowVisible"
        @closeVisible="toggleFollowVisible=false;getDemandDataList=null;$root.BackToTop();getDemandList()"
        @getFollowList="getFollowList(id)"
        :getDemandDataList="getDemandDataList"
    ...
    <!-- 需求新增 -->
    <new-demand
      :needVisible="needVisible"
      @closeNeedVisible="needVisible=false;"
    ...
    <transition name="fade-transform" mode="out-in">
      <offer
        ...
        :isReportP="isReport"
        :reportTimeP="reportTime"
        :terminalP="terminal"
        :moneyStatusP="moneyStatus"
        :followDemandList="followDemandList"
        @closeOffer="offerVisible=false;offerEditId=null;isReport=null;reportTime=null;terminal=null;
...
import { getDemadDiscountList, getGoodsList } from "util/req/offerList/index";
import {
  getSalesList,
  getCustomerConList,
  getCustomerName
...
export default {
  ...
  methods: {
    ...
    offerHandle(id, status, row, offer) {
      //打开报价弹框,获取报价列表,折扣
      if (status == 5 && offer != "offer") {
        //已生成合同需要预览PDF
      ...
      this.demandId = id;
      if (status == 1) {
        //此时为报价状态，需要获取一次申请折扣信息，查看是否有申请折扣信息
 ...
 */
          ...
          if (res.data == null) {
            ...
            colum.offerGoodsList.forEach((item, idx) => {
              ...
              partsList.forEach(item => {
                paramsList.push({
                  ...
                  finalDiscount:
                    Math.round(Number(item.finalDiscount) * 100) / 100, // 最终折扣
            ...
            var repeatArr = colum.offerGoodsList.reduce(function(item, next) {
              if (!obj[next.modelId]) {
                ...
                item[index].editPartsList =
                  item[index].editPartsList.length > 0
                    ? item[index].editPartsList
                    : next.editPartsList;
                item[index].allPartsList =
                  item[index].allPartsList.length > 0
                    ? item[index].allPartsList
                    : next.allPartsList;
            ...
            this.isReport = res.data.offerInfo.isReport; //是否报备
            this.reportTime = res.data.offerInfo.reportTime; //报备有效期
            this.moneyStatus = res.data.offerInfo.moneyStatus; //付款方式
            this.terminal = res.data.offerInfo.remark;
            console.log("tag", res.data.offerInfo);
          }
        }
      );
    },
    sendEmailHandle() {
      //发送邮件
      this.$confirm(
        "此操作发送报价单于" + this.emailSelectRow.contacts + ", 是否继续?",
        "提示",
        {
  ...
  mounted() {
    ...
    if (this.$route.query.condition) {
      this.searchCriteria = JSON.parse(
        JSON.stringify(this.$route.query.condition)
      );
    }
    if (this.$route.query.cid) {
      this.selectLinkMan = this.$route.query.cid;
      //去掉又客户需求跳转进来的需求带公司名称
      if (this.$route.query.demandNum == 0) {
        //当需求数量为0时，不弹出新建需求
    ...
    if (
      sessionStorage.getItem("buyer_department") ||
      sessionStorage.getItem("technical_department")
    ) {
      this.isShow = false;
    }
  },
  components: {
    customersContract: () =>
      import("@/page/admin/offerList/components/customersContract"),
