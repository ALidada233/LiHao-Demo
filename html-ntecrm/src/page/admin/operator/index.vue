  ...
  <div class="operator">
    <div>
      ...
      <el-button
        ...
        @click.native.prevent="searchHandle"
        icon="el-icon-search"
      ></el-button>
      <el-button
        type="primary"
        size="medium"
        style="margin-bottom: 10px;"
        @click="dialogVisible=true;searchRegion()"
        icon="el-icon-plus"
      ></el-button>
    </div>
    <div class="operatorTable">
      ...
      >
        ...
        <el-table-column fixed label="操作" width="180">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              size="mini"
              round
              @click="moveOperator(scope.row.id)"
            ></el-button>
            <el-tooltip
              :content="scope.row.status==0?'锁定':'解锁'"
              placement="top"
            ...
            <el-button
              ...
              size="mini"
              round
              @click="returnOperPwdHandle(scope.row.id)"
            ></el-button>
            <el-button
              style="margin-top:5px"
              type="danger"
              ...
              size="mini"
              round
              @click="switchChange(2,scope.row.id)"
            ></el-button>
          </template>
        </el-table-column>
        <!-- <el-table-column
        ...
        <el-table-column prop="code" label="登录名" align="center" width="100"></el-table-column>
        <el-table-column prop="name" label="姓名" align="center" width="100"></el-table-column>
        <el-table-column prop="nameabbv" label="姓名缩写" align="center" width="100"></el-table-column>
        <el-table-column prop="telphone" label="手机号" align="center" width="120"></el-table-column>
        ...
        <el-table-column prop="operatorDiscount" label="折扣率(%)" align="center" width="100"></el-table-column>
        <el-table-column prop="commissionRate" label="提成率(%)" align="center" width="100"></el-table-column>
    ...
    >
        ...
        <div>
          ...
          >
            <el-input
              ...
              maxlength="200"
              style="width: 80%;"
              @blur="checkOperatorCode()"
            ></el-input>
            <br />
            <span style="color:red">{{checkCodeMsg}}</span>
          </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" :disabled="btndisabled" @click="submitHandle">确 定</el-button>
      </span>
    </el-dialog>
    ...
    >
      <el-form>
        ...
        <el-form-item label="转移客户" :label-width="formLabelWidth">
          <el-table
            :data="cusTableData.filter(data => !search || data.custCompanyName.toLowerCase().includes(search.toLowerCase()))"
            height="400"
            @selection-change="handleSelectionChange"
            style="width: 100%"
          >
            <el-table-column type="selection" width="55"></el-table-column>
            <el-table-column prop="salesmanName" label="销售人员" width="100"></el-table-column>
            <el-table-column align="left">
              ...
              <template slot-scope="scope">
                <span>{{scope.row.custCompanyName}}</span>
              </template>
            </el-table-column>
            <el-table-column align="left">
              <template slot="header">
            ...
            <el-table-column prop="conInfo" label="联系人"></el-table-column>
          </el-table>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
import { getCustomerName } from "util/req/customer/table";
  ...
  methods: {
    ...
    comfirmMove() {
      ...
      if (this.multipleSelection.length > 0) {
        if (this.multipleSelection.length == this.cusData.length) {
          params.custId = "ALL";
        } else {
          let custId = this.multipleSelection.reduce((prev, item, next) => {
            prev.push(item.custId);
            return prev;
          }, []);
          params.custId = custId.join(",");
        }
      } else {
        this.$message("请先选中客户！");
        return;
    ...
    //查询业务员底下所有的客户
    getCustomerName() {
      getCustomerName({ salesmanId: this.selectId, type: "ALL" }, res => {
        this.search = null;
        this.cusTableData = res.data.data.list;
        this.cusData = res.data.data.list;
      });
    },
    //多选的内容
    ...
    regionChange() {
      this.cusTableData = this.cusData;
      if (
        this.regionIds != null &&
        this.regionIds != [] &&
        this.regionIds.length != 0
      ) {
        var rId = "-" + this.regionIds.join("-") + "-";
        console.log(rId);
        if (!rId.includes("-1-")) {
          this.cusTableData = this.cusTableData.filter(
            data =>
              data.regionIds != null &&
              data.regionIds != "" &&
              rId.includes("-" + data.regionIds + "-")
          );
        }
      }
    }
