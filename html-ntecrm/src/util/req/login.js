...
//登录
export const LoginFn = (user, psd, smsCode, callback) => {
  var c_user = htmlEncodeByRegExp(user);
  var c_psd = htmlEncodeByRegExp(psd);
  c_psd = md5(c_psd); //e10adc3949ba59abbe56e057f20f883e
  axios.get(api + "/toLogin.do?name=" + c_user + "&pwd=" + c_psd + "&smsCode=" + smsCode).then((res) => {
    var data = res.data;
    if (data.msgCode == 200) {
