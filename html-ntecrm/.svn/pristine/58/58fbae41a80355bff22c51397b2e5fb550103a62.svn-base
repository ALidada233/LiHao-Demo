<template>
  <el-popover placement="bottom-start" width="300" v-model="popoverState" trigger="click">
    <div class="clearPa" style="margin-bottom:10px;">
      ...
      <el-date-picker v-model="yearValue" type="year" value-format="yyyy" placeholder="选择年"></el-date-picker>
    </div>
    ...
    <el-collapse accordion style="padding:0 20px;">
      <el-collapse-item>
        <template slot="title">自定义时间</template>
    ...
    <span slot="reference" style="font-size: 12px;color:#666;margin-right: 10px;cursor:default;">
      选择时间： {{echoMsg}}
...
export default {
  ...
  methods: {
    ...
    cancleHandle() {
      ...
      // this.yearValue = String(new Date().getFullYear());
      // this.timeRadio = "本周";
    ...
    confirmHandle() {
      ...
      sessionStorage.setItem(
        "homeSelTime",
        JSON.stringify({ timeStart: params.timeStart, timeEnd: params.timeEnd })
      );
      this.popoverState = false;
    }
  },
    ...
    timeRadio(val) {
      ...
      if (this.tabIndex == 0) {
        if (this.timeRadio == "昨天") {
          ...
          var NowWeekStartDate = new Date(
            nowYear,
            nowMonth,
            nowDay - nowDayOfWeek
          );
          var NowWeekEndDate = new Date(
            nowYear,
            nowMonth,
          ...
          var lastMonthEndDate = new Date(
            newYear,
            lastMonth,
            this.getMonthDays(newYear, lastMonth)
          );
          ...
          var monthEndDate = new Date(
            nowYear,
            nowMonth,
            this.getMonthDays(this.yearValue, nowMonth)
          );
        ...
        this.timeStart = this.getQuarterStartDate(
          this.yearValue,
          this.timeRadio
        );
